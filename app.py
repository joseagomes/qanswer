...
def ask():
    ...
    db = get_db()
    if request.method == 'POST':
        db.execute('insert into questions (question_text, asked_by, expert_id) values (?, ?, ?)', [request.form['question'], user['id'], request.form['expert']])
        db.commit()
        return redirect(url_for('index'))
...
def unanswered():
    ...
    users.name from questions join users on users.id = questions.asked_by 
    where questions.answer_text is null and questions.expert_id is ?''', [user['id']])
